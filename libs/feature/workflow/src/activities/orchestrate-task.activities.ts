...
let minioBucket: string = 'opwerf-artifacts';
...
export function initActivities(deps: {
  ...
  if (deps.minioClient) minioClient = deps.minioClient;
  if (deps.minioBucket) minioBucket = deps.minioBucket;
}
    ...
    if (result.isOk() && result.value.stdout) {
      ...
      if (mirror) {
        for (const artifact of artifacts) {
          ...
          if (artifact.filePath && minioClient) {
            try {
              const fileResult = await sandboxAdapter.readFile(input.sandboxId, artifact.filePath);
              if (fileResult.isOk()) {
                const filename = path.basename(artifact.filePath);
                const objectKey = `${input.tenantId}/${input.temporalWorkflowId}/${entity.id}/${filename}`;
                const buffer = Buffer.from(fileResult.value, 'utf-8');
                await minioClient.putObject(minioBucket, objectKey, buffer, buffer.length, {
                  'Content-Type': artifact.mimeType || 'application/octet-stream',
                });
                entity.uri = `s3://${minioBucket}/${objectKey}`;
              } else {
