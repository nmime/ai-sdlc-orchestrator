...
Refactored the AI SDLC Orchestrator to be fully **provider-agnostic**. The core codebase no longer contains hardcoded references to specific AI providers (claude, openhands, aider). All provider selection is config-driven via environment variables and the `AiAgentPort` interface.
